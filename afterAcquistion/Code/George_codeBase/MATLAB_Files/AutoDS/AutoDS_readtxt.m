...
%%
function Meta = AutoDS_readtxt(dirAcq, folSink,ExNum,dirSource)
% clear; clc; close all;
%%
...
while (~foundSettingsFile)
    fprintf('Searching for Settings file for Ex%02d\n', ExNum);
    settingsSource = [dirAcq filesep '*Settings*'];
...
f = fopen([dirAcq filesep settingsFile]);
tline = fgetl(f);
% %
...
Meta.dirParent = dirSource;
dirCS = [folSink filesep txtFile.folder(idxCS: idxEx-1)];
if ~exist(dirCS, 'dir')
    mkdir(dirCS);
    tstr = getTime;
...
for ii = 1:length(Meta.chan)
    ...
    dirChan = [folSink txtFile.folder(idxR(end-1):end) filesep strChan];

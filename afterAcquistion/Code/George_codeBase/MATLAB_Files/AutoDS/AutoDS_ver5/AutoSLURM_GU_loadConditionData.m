function [data] = AutoSLURM_GU_loadConditionData(Ex_directory, Meta)
...
% ip.addOptional('chNames',{} , @iscell); %{'ch1','ch2','ch3'}
% ip.addOptional('markers', {}, @iscell); %{'alexa488','alexa568','alexa647'},
% ip.addParamValue('Parameters', [1.4 150 16e-6], @(x) numel(x)==3);
% ip.addParamValue('MovieSelector', 'Ex', @ischar);
% ip.addParamValue('StrictSelector', false, @islogical);
% ip.addParamValue('IgnoreEmptyFolders', true, @islogical);
% ip.addOptional('maxlevel', 5, @(x) isempty(x) || (isnumeric(x) && abs(round(x))==x));
...
cellPath = recursiveDir(condDir, 'maxlevel', maxlevel);

% cell directories:
[cellDirs, cellPar] = cellfun(@(i) getDirFromPath(i), cellPath, 'unif', 0);

idx = regexpi(cellDirs, MovieSelector, 'once');
...
for k = 1:numel(ui)
    if ~isempty(sortStringsByToken(cellPath(ai==k), MovieSelector, 'post')) %% added by GU
    cellPath(ai==k) = sortStringsByToken(cellPath(ai==k), MovieSelector, 'post');
    else  %% GU
        cellPath(ai==k) = []; %% GU
        ai(ai==k) = []; %% GU
    end %% GU
...
for k = 1:nCells
    ...
    for c = 1:nCh
        if ~isempty(chNames{c})
            channels{c} = [cellPath{k} char(chNames{c}) filesep];
        else

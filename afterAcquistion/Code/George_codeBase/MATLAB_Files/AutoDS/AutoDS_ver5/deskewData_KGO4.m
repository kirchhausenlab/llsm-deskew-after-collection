...
ip.addParamValue('BkImageChannels', [], @isstruct);
ip.parse(varargin{:});
...
for ii = 1:length(data.channels)
    % get channel info
    expression = [filesep 'ch(\w+)nm.*(Cam[AB])'];
    ...
    LSImage_keyCandidate = sprintf('ch%snm%s',channel, cam);
    
    BKImage{ii} = B.tif_av(cam);

    
    try
        LSImage{ii} = I.tif_av(LSImage_keyCandidate);
    catch 
        % end the for loop
        sprintf('Warning: %s key not recognized in Illum, No illumination Correction will be applied\n',LSImage_keyCandidate)
    ...
    LSImage_str{ii} = [I.dir filesep I.chan{idx_cam, idx_chan}];
    
    
  
end
disp('Illum, BK successfully processed')
...
for i = 1:nd
    if ~hasDS(i) || ip.Results.Overwrite
        ...
        for c = 1:nCh
            ...
            [~,~] = mkdir(dsPath);
            if LLFFCorrection
                LSIm = cell2mat(LSImage{c});
                BKIm = cell2mat(BKImage{c});
            ...
            if ~objectiveScan(i)
                ...
                for f = 1:nf % PARFOR
                    ...
                    frame = double(readtiff(data(i).framePaths{f})); %#ok<PFBNS>
                    
                    % sCMOS camera rotation / flip if necessary
                    
                    if ip.Results.sCMOSCameraFlip
                        
                        [~, fn, ~] = fileparts(data(i).framePaths{f});
                    ...
                    [~,fname] = fileparts(data(i).framePaths{f});
                ...
                frame = double(readtiff(data(i).framePaths{f}));
                ...
                if LLFFCorrection
                    if sum(size(frame,1:2) > size(LSIm,1:2)) > 1
                        ...
                        str = char(regexp(char(LSImage_str{c}),expression,'match'));
    ...
    for i = 1:nd
        if ~hasDSR(i) || ip.Results.Overwrite
            ...
            for c = 1:numel(data(i).channels)
                ...
                nf = data(i).movieLength;
                parfor f = 1:nf
                    [~,fname] = fileparts(data(i).framePaths{f}); %#ok<PFBNS>

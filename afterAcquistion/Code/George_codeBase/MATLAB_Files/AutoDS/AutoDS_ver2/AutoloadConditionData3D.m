...
function [data] = AutoloadConditionData3D(dirParent)
...
% ip.parse(varargin{:});
dirParent = char(dirParent);

data = AutoloadConditionData(dirParent);
nd = numel(data);
...
%     data(i).timestamps = hour(t)*3600 + minute(t)*60 + second(t);
    data(i).timestamps = floor((t*3600 / 60)) + floor(t * 60) + floor(t);
...
for i = 1:nd
    ...
    for c = 1:nCh
        ...
        data(i).framePathsGPUDecon{c} = cellfun(@(f) [data(i).channels{c} 'DS' filesep 'GPUdecon' filesep  f ext], fname, 'unif', 0);
    end
end

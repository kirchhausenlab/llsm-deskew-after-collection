...
% % loop through the channels, find which channel, cam to use for this [data]
% for ii = 1:numel(data)
%     dataframePathsDSi = data.framePathsDS{ii};
%     cam = regexpi(dataframePathsDSi, Cam);
%     chan = regexpi(dataframePathsDSi, [ Chan]);
%     camidx = ~cellfun(@isempty,cam);
%     chanidx = ~cellfun(@isempty,chan);
%
%     if sum(camidx) + sum(chanidx) ~= 2
%         LLFFCorrection = false;
%         disp('Could not find appropriate LLSM correction tif. No LLFF will be carried out');
%     else
%         if ~isempty(I) && ~isempty(B)
%             LSImage{ii} = [I.dir filesep I.chan{camidx, chanidx}];
%             BKImage{ii} = B{camidx};
%             LLFFCorrection = true;
%         else
%             LLFFCorrection = false;
...
% disp('In Z://'); % comment george may 18th 2020

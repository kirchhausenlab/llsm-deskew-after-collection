function sinkEx = transferLocalToSink(dirSink, Ex)
% Assume [Ex] is coming from llsm, ao or scratch
    ...
    % if the directory has the name 'raw'
    if ~isempty(regexpi(dirSink,'/raw','match')) 
        transfer_flag = {
            [dirSink filesep 'tranfer.txt']
            [dirSink filesep 'tranfering.txt']
            [dirSink filesep 'tranfered.txt']
            };
    
        % if the transfer flag is not there
        counter = 0;
        for ii = 1:length(transfer_flag)
             if ~isempty(exist(char(transfer_flag{ii}),'file'))
                 counter = counter + 1;
             end
        end
        
       if counter > 2
            save(fullfile([dirSink filesep],'transfer.txt'));
       end

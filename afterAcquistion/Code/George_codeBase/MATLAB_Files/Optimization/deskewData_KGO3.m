    ...
    if ~hasDS(i) || ip.Results.Overwrite
        ...
        for c = 1:nCh
            ...
            if ~objectiveScan(i)
                ...
                disp('Detected Objective Scan!!!');
                fprintf('Applying Illum Correction only, No deskew ''%s'', channel %d ... ', getShortPath(data(i)), c);
                save(fullfile(dsPath,'No_Deskew_Applied.txt'));
                for f = 1:nf
                    % load frame
                    frame = double(readtiff(data(i).framePaths{c}{f})); %#ok<PFBNS>
                    if ip.Results.sCMOSCameraFlip

function applyChromaticOffset2(data, Meta, C)
...
for ii = 1:length(data.channels)
    ...
    Chroma_keyCandidate = sprintf('ch%snm%s',channel, cam);
    
    try
        XYZz(ii) = C.values(Chroma_keyCandidate);
    catch
        % end the for loop
        sprintf('Warning: %s key not recognized in Illum, No Chormatic Correction will be applied\n',Chroma_keyCandidate)
        C.doChroma = false;
        break
    end
